...
public class YoRPG
{
	...
	if (job == 1) pat = new Warrior( name );
	if (job == 2) pat = new Mage( name );
	if (job == 3) pat = new Rogue( name );
	if (job == 4) pat = new Archer( name );
	if (job == 5) pat = new Cleric( name );
	
        if (job > 5) {
	    System.out.println("\nThou art such a meathead, thou best befits a Warrior!");
